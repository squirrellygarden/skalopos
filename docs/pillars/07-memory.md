...
- DEP (W^X) and ASLR are not configurable — they are always on.
...
## Contract — anonymous and file-backed memory
...
- Pass the handle in the `handles_to_pass` array of [`proc_spawn`](02-spawn.md), or
...
- **DEP / W^X** — always on. Hardware NX/XN bit set on all writable pages. `vm_alloc` and `vm_protect` reject `WRITE | EXEC`. The kernel's `.text` and the userland's `.text` are also never writable.
...
- **W^X opt-in** — Linux did this for years; lots of code accidentally allocated RWX. v1 Skalapos enforces by default; if a future JIT genuinely needs it, we add `vm_alloc_unsafe` then.
- **Variable ASLR strength** — pointless tunable. On is on.

## v2+ direction

- **Real allocator backing libc `malloc`.** v1 ships a bump allocator; v2 or v3 adds a real one (dlmalloc-style or a slab+bins design). Same libc surface.
...
- **Huge pages.** Performance work; not for a toy OS.
- **`mlock`-equivalent.** Pin pages in RAM. Useful for crypto code; not v1.
- **KASAN-style instrumented mode.** Optional kernel build flag.
